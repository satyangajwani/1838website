...
- R6. **SUPERSEDED in part, 2026-08-20:** the owner directed the page-level mark be the brand gold `#d8b273` and larger, knowingly overriding the white/no-tint clause; Visa sign-off added to go-live. Balance of R6 (opacity, clear space, no blur/crop, ≥1 s hold) stands. Original: The Visa Brand Mark renders in white at full opacity, never blurred, cropped, tinted or obscured, with clear space of 1X on all sides; the card image holds still for at least one second in any animation.
...
- OQ1. **RESOLVED 2026-08-20.** The brand team supplied the full collateral kit (`collaterals/`): the layered pedestal mockup PSD, the archival scan of the Krishen Khanna painting (5238×7019), the open card artwork (.ai/PDF), card front and back, all four logos as vectors, the brand fonts (STIX Two Text + Montserrat, both OFL), the TOI 1838 masthead archives, and the approved launch print ad. Print-resolution layer extractions live in `assets/`.
...
- OQ3. **RESOLVED 2026-08-20.** The supplied card back reads **Visa Infinite Privilege**. The approved card front carries no Visa mark (it lives on the back), so per Visa's text rule the page names the full product — 'The 1838 Reserve Credit Card · Visa Infinite Privilege' — alongside the white page-level mark (R6).
...
- OQ6. Half resolved. October 2026 is printed in the public launch ad, so the timing claim is approved copy. The private-office name on the confirmation (R13) still needs an owner; placeholder copy remains marked in the component.
...
- KTD13. **Visa's standards are a constraint on the motion design, not a review step afterwards.** *(2026-08-20: owner knowingly overrode the white-mark clause — see R6 note; the motion constraints here still govern.)* The mark is white on this ground — a gold Visa mark is prohibited, which the live page already violates. Depth-of-field blur across the mark is an explicit "never", so the card cannot sit in shallow focus. Any animation must land a still hold of at least one second on the card image. If the art direction ultimately wants the mark smaller than legibility allows, the escape hatch is naming the full Visa product in text instead. Governs R6.
